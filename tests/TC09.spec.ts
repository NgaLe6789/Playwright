...
test('Verify users can update quantity of product in cart', async ({ page, clearCart }) => {
  ...
  await cartPage.checkQuantityProduct(product.title, 1);

  // 7. Click on Plus(+) button
  await cartPage.clickPlusQuantity(product.title);

  // 8. Verify quantity of product and SUB TOTAL price
  await cartPage.checkQuantityProduct(product.title, 2);
  await cartPage.checkSubTotal(product.title, product.price * 2);
  ...
  await cartPage.checkQuantityProduct(product.title, 4);
  await cartPage.checkSubTotal(product.title, product.price * 4);

  // 11. Click on Minus(-) button
  await cartPage.clickMinusQuantity(product.title);

  // 12. Verify quantity of product and SUB TOTAL price
  await cartPage.checkQuantityProduct(product.title, 3);
  await cartPage.checkSubTotal(product.title, product.price * 3);
